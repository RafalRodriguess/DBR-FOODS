...
const Modal: React.FC<ModalProps> = ({ open, title, children, onClose, footer }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <button className="absolute inset-0 bg-black/30" onClick={onClose} aria-label="Fechar modal" />
      <div className="relative w-full max-w-md bg-white rounded-2xl border border-gray-100 shadow-xl overflow-hidden">
        {title ? (
          ...
          <button onClick={onClose} className="absolute top-3 right-3 w-8 h-8 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors" aria-label="Fechar">
