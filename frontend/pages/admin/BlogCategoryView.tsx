...
import { readCategory } from '../../utils/blogApi';
  ...
  if (!cat) {
    return (
      <section className="bg-white rounded-[2rem] border border-gray-100 p-8">
        ...
        <Link to="/admin/blog/categories" className="text-gold font-bold mt-4 inline-block">Voltar</Link>
      </section>
    );
  }
    ...
    <section className="bg-white rounded-[2rem] md:rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
      <div className="p-6 md:p-8 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">Categoria do blog #{cat.id}</p>
        </div>
        <Link
          to={`/admin/blog/categories/${cat.id}/edit`}
